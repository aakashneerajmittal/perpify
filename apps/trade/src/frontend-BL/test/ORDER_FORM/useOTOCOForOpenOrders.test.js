...
describe("OCO Orders", () => {
  ...
  it("otoco test Sell LIMIT Take Profit", () => {
    ...
    act(() => result.current.handleChangeUpdatedPrice({ target: { value: "0" } }));
    act(() => result.current.handleCheckClick());
    expect(result.current.priceError).toBe("Price can not be zero");
    act(() => result.current.handleChangeUpdatedPrice({ target: { value: "46500" } }));
    act(() => result.current.handleCheckClick());
    expect(result.current.getEstimatedPnL()).toBe("--/--");
    expect(result.current.priceError).toBe("TP value should be less than the Limit price of the parent order");
    act(() => result.current.handleChangeUpdatedPrice({ target: { value: "45500" } }));
    act(() => result.current.handleCheckClick());
    expect(result.current.getEstimatedPnL()).toBe("0 USDT");
    expect(result.current.priceError).toBe("");
    act(() => result.current.handleChangeUpdatedPrice({ target: { value: "43500" } }));
    act(() => result.current.handleCheckClick());
    expect(result.current.getEstimatedPnL()).toBe("6 USDT");
    expect(result.current.priceError).toBe("");
  });
  ...
  it("otoco test Sell LIMIT Stop Loss", () => {
    ...
    act(() => result.current.handleCheckClick());
    expect(result.current.getEstimatedPnL()).toBe("--/--");
    expect(result.current.priceError).toBe("SL value should be greater than the Limit price of the parent order");
    act(() => result.current.handleChangeUpdatedPrice({ target: { value: "45500" } }));
    act(() => result.current.handleCheckClick());
    expect(result.current.getEstimatedPnL()).toBe("0 USDT");
    expect(result.current.priceError).toBe("");
    act(() => result.current.handleChangeUpdatedPrice({ target: { value: "48500" } }));
    act(() => result.current.handleCheckClick());
  ...
  it("otoco test Buy LIMIT Stop Loss", () => {
    ...
    act(() => result.current.handleChangeUpdatedPrice({ target: { value: "47500" } }));

    act(() => result.current.handleCheckClick());
    expect(result.current.getEstimatedPnL()).toBe("--/--");
    expect(result.current.priceError).toBe("SL value should be less than the Limit price of the parent order");

    act(() => result.current.handleChangeUpdatedPrice({ target: { value: "42500" } }));
    act(() => result.current.handleCheckClick());
    expect(result.current.getEstimatedPnL()).toBe("9 USDT");
    expect(result.current.priceError).toBe("");
  });
  ...
  it("otoco test Buy LIMIT take profit", () => {
    ...
    expect(result.current.getEstimatedPnL()).toBe("6 USDT");
    act(() => result.current.handleChangeUpdatedPrice({ target: { value: "41500" } }));

    act(() => result.current.handleCheckClick());
    expect(result.current.getEstimatedPnL()).toBe("--/--");
    expect(result.current.priceError).toBe("TP value should be greater than the Limit price of the parent order");

    act(() => result.current.handleChangeUpdatedPrice({ target: { value: "49000" } }));
    act(() => result.current.handleCheckClick());
  ...
  it("otoco test Buy STOP SL", () => {
    ...
    act(() => result.current.handleChangeUpdatedPrice({ target: { value: "45000" } }));
    act(() => result.current.handleCheckClick());
    expect(result.current.getEstimatedPnL()).toBe("6 USDT");
    expect(result.current.priceError).toBe("");
  });
  ...
  it("otoco test Buy Take profit market TP", () => {
    ...
    act(() => result.current.handleCheckClick());
    expect(result.current.getEstimatedPnL()).toBe("--/--");
    expect(result.current.priceError).toBe("TP value should be greater than the trigger price of the parent order");

    act(() => result.current.handleChangeUpdatedPrice({ target: { value: "46000" } }));
    act(() => result.current.handleCheckClick());
  ...
  it("otoco test SELL STOP Market SL", () => {
    ...
    act(() => result.current.handleCheckClick());
    expect(result.current.getEstimatedPnL()).toBe("--/--");
    expect(result.current.priceError).toBe("SL value should be greater than the trigger price of the parent order");
  ...
  it("otoco test BUY take profit Market SL", () => {
    ...
    expect(result.current.getEstimatedPnL()).toBe("6 USDT");
    act(() => result.current.handleChangeUpdatedPrice({ target: { value: "44500" } }));

    act(() => result.current.handleCheckClick());
    expect(result.current.getEstimatedPnL()).toBe("--/--");
    expect(result.current.priceError).toBe("SL value should be less than the trigger price of the parent order");
  ...
  it("otoco test SELL Stop Market SL", () => {
    ...
    act(() => result.current.handleCheckClick());
    expect(result.current.getEstimatedPnL()).toBe("--/--");
    expect(result.current.priceError).toBe("TP value should be less than the trigger price of the parent order");
