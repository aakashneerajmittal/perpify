...
describe("TakeProfit stopLoss order for orderForm", () => {
  ...
  it("SizeField state order type market test when quantity in contract  will change", async () => {
    ...
    expect(result.current.quantityValue).toBe(convertToPrecisionValueInPrecisionUnit(Number(store.getState().OrderBook.asks[0][0]) * (1 + 0.0005) * 0.003, 1));
  });
  ...
  it("SizeField state test set min quantity ordertype market and quantity type is usdt will change", async () => {
    ...
    expect(result.current.minQantityForContract).toBe(
      Math.max(minQTY, Number(convertToPrecisionValueInPrecisionUnit((Number(notionValue) / Number(store.getState().OrderBook.asks[0][0])) * (1 + 0.0005), Number(precision))) + Number(stepSize))
    );
  });
  it("SizeField state test set min quantity ordertype market and quantity type is usdt will change", async () => {
    const state = {
