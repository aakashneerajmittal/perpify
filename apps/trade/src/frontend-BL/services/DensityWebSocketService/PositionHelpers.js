...
export function removeOrderFromSnapshot(orderID, store) {
  ...
  const openOrdersApiDataAfterRemoval = openOrdersApiData.filter((openOrder) => openOrder.ID !== orderID);
...
export function updateFilledSizeForSnapshot(stream, store) {
  const openOrdersApiData = store.getState().futures.openOrders;
  const openOrdersApiDataAfterFilledSize = openOrdersApiData.map((openOrder) => (openOrder.orderId === stream.i ? { ...openOrder, executedQty: openOrder.executedQty + stream.l } : openOrder));
  store.dispatch({
    type: OPEN_ORDERS_UPDATE_SIZE_REST,
