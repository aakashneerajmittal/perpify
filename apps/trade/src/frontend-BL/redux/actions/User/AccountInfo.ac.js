...
export const fetchAccountPositionInfo = (TracePositionDatabyId, store) => (dispatch) => {
  ...
  getFuturesPosition().then(
    (result) => {
      ...
      const activePositionsArr = result.data.positions.filter((item) => Number(item.positionAmount) !== 0);
      for (let i = 0; i < activePositionsArr.length; i++) {
        const activePositionsPayload = {
          ...
          isolatedWallet: activePositionsArr[i].isolatedWallet || 0,
        ...
        dispatch({
          ...
          payload: {
            sym: activePositionsArr[i].symbol,
            isolatedWallet: activePositionsArr[i].isolatedWallet
          }
        });
        dispatch({
