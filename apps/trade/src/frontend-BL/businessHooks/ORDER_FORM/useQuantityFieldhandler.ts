...
const useQuantityFieldhandler = ({ state, dispatchOrderEvent }: { state: any; dispatchOrderEvent: any }) => {
  ...
  const { assumingPrice } = useMarketBestPricehandler({ side: state.side, symbol });
    ...
    if (symbol.length && leverageBracketList.length) {
      ...
      if (leverageBracketListForSelectedSymbol.length > 0) {
        dispatchOrderEvent({
          ...
          payload: leverageBracketListForSelectedSymbol[0]?.brackets[0].initialLeverage
        });
      }

      const quantityValueInNumber = state.size ? parseFloat(state.size) : 0;
      for (let bracketList = 0; bracketList < leverageBracketListForSelectedSymbol[0]?.brackets.length; bracketList++) {
        if (
          quantityValueInNumber >= leverageBracketListForSelectedSymbol[0].brackets[bracketList].notionalFloor &&
          quantityValueInNumber < leverageBracketListForSelectedSymbol[0].brackets[bracketList].notionalCap
        ) {
          dispatchOrderEvent({
            type: "UPDATE_MAX_LEVERAGE_SIZE",
            payload: leverageBracketListForSelectedSymbol[0].brackets[bracketList].initialLeverage
          });
  ...
  const handleLastPrice = (payload: string) => {
    const value = String(assumingPrice);
    if (payload === "LIMIT") {
