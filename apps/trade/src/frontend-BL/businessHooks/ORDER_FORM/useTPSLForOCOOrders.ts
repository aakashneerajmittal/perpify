...
const useTPSLForOCOOrders = ({ symbol, side, size, entryPrice, costValue, close = () => {}, isSignalTrading }: UseTakeProfitProps) => {
  ...
  const calculateEstimateProfitAndLossForTakeProfit = useCallback(
    (payload: string, entryPrice: number, size: any, side: string) => {
      if (side === "BUY") {
        const Pnl = Number(size) * (Number(payload) - Number(entryPrice));
        if (Pnl > 0) {
          setEstimateProfitForTakeProfit(convertToPrecisionValueInContractAssetUnit(String(Pnl), symbolPrecisionByMinTickSize));
        } else {
          setEstimateProfitForTakeProfit("");
        }
      } else {
  ...
  const calculatedMaxPercentangeProfitLoss = (ltp: string, type: "SL" | "TP") => {
    ...
    return convertToPrecisionValueInContractAssetUnit((Number(amount) * 100) / Number(costValue), 2);
  };
  ...
  const calculatedMinProfitAmount = (ltp: string) => {
    ...
    return convertToPrecisionValueInContractAssetUnit(amount, symbolPrecisionByMinTickSize);
  };
    ...
    switch (activeStep) {
      ...
      case 4:
        return {
          ...
          primaryAction: () =>
            cancelOrder({
              orders: strategyOrders.ocoOrders[index - 1],
              setLoading: () => {}
            }),
          secondaryAction: () => setActiveStep(0)
        };
