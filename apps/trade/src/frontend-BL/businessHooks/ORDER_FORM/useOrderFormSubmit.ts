...
const useOrderFormSubmit = ({
  ...
  const leverageFromServer = useSelector((state) => state.positionsDirectory.leverage).find((item: { sym: string }) => item.sym?.toUpperCase() === symbol.toUpperCase());
  ...
  const createOrderApiCall = () => {
    ...
    createOrder(formValues, dispatch, setShowLoader, setOrderConfirm, positionsTabNavCallback, setOrderStatus, setOrderErrors);
