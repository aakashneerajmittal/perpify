...
import { updateOCOOrder, cancelAllStrategyOrdersApi, getMarginTypeApi } from "../../../frontend-api-service/Api";
import { GLOBAL_ERROR_ADD, GLOBAL_ERROR_REMOVE, ORDER_CREATION_TP_SL_SUCESS, ORDER_CREATION_TP_SL_FAIL, ORDER_CREATION_FAIL, ORDER_CREATION_SUCESS } from "../../redux/constants/Constants";
...
export const useOtOCOForOpenOrders = ({ data, parentData, childOrderType, close }: UseOtOCOForOpenOrdersProps) => {
  ...
  useEffect(() => {
    if (parentData.symbol) {
      getMarginTypeApi(parentData.symbol.toUpperCase()).then((marginType: { data: { marginType: string } }) => setMarginType(marginType.data.marginType));
    }
  }, [parentData]);

  const [loading, setLoading] = useState(false);

  const pricePrecisionValue = useSelector((state: { tradablesymbolList: { tradablesymbolList: TradableSymbol[] } }) =>
    state.tradablesymbolList.tradablesymbolList.find((item: TradableSymbol) => item.symbol.toLowerCase() === parentData?.symbol.toLowerCase())
  );
  ...
  const handleCheckClick = () => {
    ...
    updateOCOOrder(payload)
          ...
          payload: {
            ...
            errorHandlerForReduxStateUpdation: () =>
              dispatch({
                ...
                payload: { src: ORDER_CREATION_TP_SL_SUCESS }
              })
          }
        });
        setLoading(false);
      ...
      .catch((err: { response: { data: { details: string }; status: number } }) => {
        dispatch({
          type: GLOBAL_ERROR_ADD,
          payload: {
            ...
            errorHandlerForReduxStateUpdation: () =>
              dispatch({
                type: GLOBAL_ERROR_REMOVE,
                payload: { src: ORDER_CREATION_TP_SL_FAIL }
              })
          }
        });
        setLoading(false);
  ...
  const cancelAllStrategyOrders = () => {
    if (!parentData) return;
    setLoading(true);
    cancelAllStrategyOrdersApi(parentData.symbol)
      .then((successResponse: { status: number }) => {
        dispatch({
          type: GLOBAL_ERROR_ADD,
          payload: {
            ...
            errorHandlerForReduxStateUpdation: () =>
              dispatch({
                type: GLOBAL_ERROR_REMOVE,
                payload: { src: ORDER_CREATION_SUCESS }
              })
          }
        });
        setLoading(false);
      ...
      .catch((errorResponse: { response: { data: { details: string }; status: number } }) => {
        dispatch({
          type: GLOBAL_ERROR_ADD,
          payload: {
            ...
            errorHandlerForReduxStateUpdation: () =>
              dispatch({
                type: GLOBAL_ERROR_REMOVE,
                payload: { src: ORDER_CREATION_FAIL }
              })
          }
        });
        setLoading(false);
