      ...
      {TransactionHistoryData.map((txn: any) => {
        return (
          ...
          <Grid container mt={2}>
            ...
            <TradeIDcell tradeID={txn?.tradeID} gridSize={2.5} />
            <Grid item container xs={2.5} alignItems={"center"} gap={2}>
              <TextView variant="Regular_14" color={"text.regular"} text={"******" + txn?.transactionID?.substr(-6)}></TextView>
              <CopyButton copyText={txn?.transactionID} />
            </Grid>
          </Grid>
        );
      })}
