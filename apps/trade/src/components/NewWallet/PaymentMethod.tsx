...
const PaymentMethod = ({ isChecked, setPaymentMethod, method }: { isChecked: boolean; setPaymentMethod: (val: string) => void; method: string }) => {
  return (<>
    ...
    <TextView variant={"Regular_12"} color={"text.main"}>{"** Our deposit service is currently down for maintenance. We're working hard to get it back up and running."}</TextView>
    </>
  );
};
