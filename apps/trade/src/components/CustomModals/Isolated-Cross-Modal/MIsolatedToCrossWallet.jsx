...
const MIsolatedToCrossWallet = ({ setMarginType, marginType, symbol = "BTCUSDT", IsOpen, close }) => {
  ...
  const allPositionsData = useSelector((state) => state.currentPositions.currentPositions);
  const [renderType, setRenderType] = useState("");
  const openOrdersData = useSelector((state) => state.futures.openOrders);

  const getPositionAndOrderIndex = () => {
    // check positions array and open orders array for symbol
    const positionIndex = allPositionsData.findIndex((activePosition) => activePosition.sym === symbol.toUpperCase());
    const openOrderIndex = openOrdersData.findIndex((activeOrder) => activeOrder.symbol === symbol.toUpperCase());
  ...
  const callBack = (symbol, marginType) => {
    dispatch({
  ...
  const handleIsolatedToggleExpansion = (event) => {
    setExpandIsolatedContent(!expandIsolatedContent);
    setExpandCrossContent(false);
    event.stopPropagation();
  };

  const handleCrossToggleExpansion = (event) => {
