...
const ChildOrders: React.FC<ChildOrdersProps> = ({ type, gridSize, parentData, data, close }) => {
  ...
  // const [updatedPrice, setUpdatedPrice] = useState(data.stopPrice);
  useEffect(() => {
  ...
  //       pnl = Number(price) - Number(updatedPrice);
  //     } else {
  //       pnl = Number(updatedPrice) - Number(price);
  //     }
  //     if (pnl > 0) return "--/--";
  ...
  //     let pnl = 0;
  //     if (parentData.side === "SELL") {
  //       pnl = price - Number(updatedPrice);
  //     } else {
  //       pnl = Number(updatedPrice) - price;
  //     }
  //     if (pnl < 0) return "--/--";
  //     else return `${convertToPrecisionValueInContractAssetUnit(String(pnl * Number(parentData.quantity)), Number(symbolPricePrecision))} USDT`;
  //   }
