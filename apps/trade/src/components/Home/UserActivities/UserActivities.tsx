...
const UserActivities = () => {
  ...
  useEffect(() => {
    if (isLoggedIn) {
      dispatch(getLeverage());
      dispatch(fetchAccountPositionInfo());
      fetchAllOpenOrdersApi().then((openOrders: { data: { data: any } }) => {
        dispatch({
  ...
  const handleChange = (e) => {
    ...
    if(Number(attributeValue) === 3){
      recordCleverTapEvent("POSITION_HISTORY_CLICKED", {});
    }
    if(attributeValue === 1){
  ...
  return (
    <Box sx={{ backgroundColor: "text.light", borderRadius: "8px" }}>
      <Grid
        container
        sx={{
          height: "60px",
