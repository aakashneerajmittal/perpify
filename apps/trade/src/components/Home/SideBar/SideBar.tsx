...
import { getProfileApi, _24hrTicker, openInterestApi } from "@/frontend-api-service/Api";
...
import CongratulationsModal from "@/components/CustomModals/CongratulationsModal";
import WelcomeModal from "@/components/CustomModals/WelcomeModal";
...
const SideBar = () => {
  ...
  const getOpenInterestValue = (selectValue: any) => {
    openInterestApi(selectValue)
      .then((successResponse: { data: any }) => dispatch(getOpenInterest(true, successResponse.data)))
      .catch((error: any) => {
  ...
  useEffect(() => {
    ...
    const symbol = JSON.parse(localStorage.getItem("orderFormData"))?.selectedSymbol;
  ...
  // Onboarding components and state
