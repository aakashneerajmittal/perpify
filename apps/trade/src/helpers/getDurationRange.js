const getDurationRange = (duration) => {
  ...
  let from;
  const timeElapsedInaDay = (to - localTimeZoneOffset) % (24 * 3600 * 1000);
  switch (duration) {
    case "day":
    case "1 Day":
    ...
    case "month":
    case "1 Month":
      from = new Date(now.getFullYear(), now.getMonth() - 1, now.getDate()).getTime() + timeElapsedInaDay; // Subtracting 1 month in milliseconds
      break;
    case "three-months":
    case "3 Month":
      from = new Date(now.getFullYear(), now.getMonth() - 3, now.getDate()).getTime() + timeElapsedInaDay; // Subtracting 3 months in milliseconds
      break;
    case "all":
    case "All":
      return { from: "", to: "" };
