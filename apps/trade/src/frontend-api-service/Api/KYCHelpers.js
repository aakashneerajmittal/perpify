import {
  ...
  REGENERATE_KYC,
  UPDATE_KYC_DETAILS,
  CHECK_USER_NAME_VALIDATION,
  CHECK_USER_IFSC_VALIDATION
} from "../URI";
...
export const CheckBankUserName = (payload) => {
  const url = Format(CHECK_USER_NAME_VALIDATION.url);
  return axiosWithApiServer({
    url: `${url}?name=${payload}`,
    method: CHECK_USER_NAME_VALIDATION.reqType
  });
};
export const CheckIFSCCode = (payload) => {
  const url = Format(CHECK_USER_IFSC_VALIDATION.url);
  return axiosWithApiServer({
    url: `${url}?ifsc=${payload}`,
    method: CHECK_USER_IFSC_VALIDATION.reqType
  });
};
