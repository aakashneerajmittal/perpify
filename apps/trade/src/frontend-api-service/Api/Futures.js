...
export const fetchAllOpenOrdersApi = () => {
  const url = Format(FETCH_ALL_OPEN_ORDERS.url);
...
export const getfuturesTransactionhistory = (payload) => {
  const url = Format(TRANSACTION_HISTORY.url);
  return axiosWithApiServer({
    url: `${url}?incomeType=${payload?.incomeType}&page=${payload?.page}&size=${payload.size}&startTime=${payload.startTime}&endTime=${payload.endTime}&symbol=${payload.symbol}`,
...
export const closePositionApi = (payload) => {
  const url = CLOSE_ORDER.url;
  return axiosWithApiServer({
    url,
...
// export const fetchPortfolioStatisticsLifetimeApi = (lifeTime) => {
//   const url = Format(PORTFOLIO_STATISTICS_LIFETIME.url, lifeTime);
//   return axiosWithApiServer({
//     url,
//     method: PORTFOLIO_STATISTICS_LIFETIME.reqType
//   });
// };
...
export const cancelAllStrategyOrdersApi = (symbol) => {
  const url = Format(CANCEL_ALL_STRATEGY_ORDER.url, symbol);
  return axiosWithApiServer({ url, method: CANCEL_ALL_STRATEGY_ORDER.reqType });
};
